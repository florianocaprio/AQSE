...
def _feature_record(
    ...
    *,
    ...
    return State8FeatureRecord(
        window_id=f"{plan.episode_id}:{profile_id}:{plan.focal_sensor_id}:1800:2200",
        ...
        start_time_s=18.0,
        end_exclusive_time_s=22.0,
        source_frame_ids=tuple(range(1801, 2201)),
    ...
    for plan in network_study_plans:
        observations.append(
            NetworkStudyObservation(
                ...
                local_feature=_feature_record(plan, network=False),
                network_feature=(
                    _feature_record(plan, network=True)
